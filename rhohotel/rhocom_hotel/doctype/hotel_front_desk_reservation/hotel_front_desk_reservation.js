// // Hotel Front Desk Reservation - Client Script v3 FIXED
// // ✅ FIXED: Room dropdown is DISABLED - rooms added only via "Add Available Rooms" button
...
//         // ✅ DISABLE ROOM DROPDOWN COMPLETELY
//         // Users must use "Add Available Rooms" button to add rooms
//         frm.set_query('room_number', 'rooms', function(doc, cdt, cdn) {
//             // ALWAYS return __none__ - disable dropdown completely
//             // This forces users to use the "Add Available Rooms" button instead
...
//         // ✅ NEW: Edit Guest Details button (for submitted reservations)
...
//      * ✅ Shows all rooms with reservations
//      * ✅ Shows real-time status (Created ✓ or Not Created)
//      * ✅ Shows check-in status (Checked In ✓ or Not Checked In)
//      * ✅ Allows checking in without filtering
//      */
...
//                 options: `<div class="alert alert-info">
//                     <strong>Check In Rooms</strong><br>
//                     Select which rooms to check in. You can only check in rooms that don't already have check-ins.
//                 </div>`
//             },
//             {
//                 fieldname: 'rooms_html',
...
//                             // ✅ NEW: Get check-in status for all rooms
//                             frappe.call({
//                                 method: 'frappe.client.get_list',
//                                 args: {
...
//                                                 check_in_datetime: checkin.check_in_datetime
//                                             };
//                                         });
//                                     }
...
//                                         checkin_status: checkins_in_db[room.room_number]?.status || null,
//                                         checkin_time: checkins_in_db[room.room_number]?.check_in_datetime || null
//                                     }));
                                    
//                                     // Build HTML - showing ALL rooms with reservation AND check-in status
//                                     let html = `
//                                         <table class="table table-bordered table-hover" style="margin: 15px 0;">
//                                             <thead>
//                                                 <tr>
...
//                                     rooms_list.forEach(function(room) {
//                                         let reservation_badge = room.has_reservation 
//                                             ? '<span class="badge badge-success"><i class="fa fa-check"></i> Created</span>'
//                                             : '<span class="badge badge-secondary">Not Created</span>';
                                        
//                                         // ✅ NEW: Check-in status badge
//                                         let checkin_badge = '';
//                                         let is_disabled = '';
                                        
//                                         if (room.has_checkin) {
//                                             checkin_badge = `<span class="badge badge-info"><i class="fa fa-check"></i> ${room.checkin_status}</span>`;
//                                             // Disable checkbox if already checked in
//                                             is_disabled = 'disabled';
//                                         } else {
//                                             checkin_badge = '<span class="badge badge-warning">Not Checked In</span>';
//                                         }
                                        
//                                         let checkbox_html = room.has_checkin 
//                                             ? `<input type="checkbox" data-room-idx="${room.index}" disabled title="Already checked in">`
//                                             : `<input type="checkbox" data-room-idx="${room.index}">`;
                                        
//                                         html += `
//                                             <tr ${room.has_checkin ? 'style="opacity: 0.6;"' : ''}>
//                                                 <td>${checkbox_html}</td>
//                                                 <td><strong>${room.room_number}</strong></td>
...
// // ═════════════════════════════════════════════════════════════════════════
// // EDIT GUEST DETAILS BUTTON & DIALOG
// // ═════════════════════════════════════════════════════════════════════════

// function show_edit_guest_details_dialog(frm) {
//     /**
//      * Dialog to edit individual guest details
//      * Users can change:
//      * - Guest Name (required)
//      * - Email (optional - if not provided, uses primary_guest_email)
//      * - Phone Number (optional - if not provided, uses primary_guest_phone)
//      * 
//      * Auto-creates/gets customer account and hotel guest record
//      */
...
//             fieldname: 'instructions',
//             options: `<div class="alert alert-info">
//                 <strong>Edit Guest Details</strong><br>
//                 Click on any room below to edit guest information. 
//                 Only Guest Name is required. Email and Phone will default to primary guest if not provided.
//             </div>`
//         },
//         {
//             fieldname: 'rooms_html',
...
//                 <tr>
//                     <td><strong>${room.room_number}</strong></td>
//                     <td>${room.guest_name || '(Not set)'}</td>
//                     <td>${room.guest_email || '(Not set)'}</td>
//                     <td>${room.guest_phone || '(Not set)'}</td>
//                     <td>
//                         <button class="btn btn-sm btn-primary edit-guest-btn" data-room-idx="${room.index}">
...
//             let room_idx = $(this).data('room-idx');
//             show_single_guest_edit_dialog(frm, room_idx, d);
//         });
//     }
...
//     /**
//      * Dialog to edit single guest details
//      * room_idx: Index of room in frm.doc.rooms array
//      */
...
//                 fieldtype: 'HTML',
//                 fieldname: 'info_html',
//                 options: `<div class="alert alert-secondary">
//                     <small>
//                         <strong>Note:</strong> If Email or Phone are left empty, the primary guest email/phone will be used.
//                     </small>
//                 </div>`
//             }
//         ],
...
//             // Call backend to update guest details
//             frappe.call({
...
//                         });
//                         d.hide();
//                         // Reload to show updated details in parent dialog
//                         frm.reload_doc();
//                     }
//                 }
//             });
...
// // Hotel Front Desk Reservation - Client Script v6 (FIXED)
// // ✅ SUPPORTS: Pre-check-in AND checked-in reservations
// // ✅ FEATURES: Extension AND reduction with smart validation
// // ✅ FIXED: Alert notifications working properly

// frappe.ui.form.on('Hotel Front Desk Reservation', {
//     refresh: function(frm) {
//         add_custom_buttons(frm);
        
//         // ✅ Add "Adjust Stay" button for both pre-check-in and checked-in
...
//         // DISABLE ROOM DROPDOWN - Users must use "Add Available Rooms" button
//         frm.set_query('room_number', 'rooms', function(doc, cdt, cdn) {
//             return { filters: { name: '__none__' } };
//         });
//     },
...
// // ═════════════════════════════════════════════════════════════════════════
// // ADJUST STAY BUTTON & DIALOG (DUAL-MODE: PRE-CHECK-IN & CHECKED-IN)
// // ═════════════════════════════════════════════════════════════════════════

// /**
//  * Add "Adjust Stay" button
//  * ✅ Visible for BOTH pre-check-in and checked-in reservations
//  */
// function add_adjust_stay_button(frm) {
//     if (frm.doc.docstatus === 1) {
//         // Button visible for ALL submitted reservations
...
// /**
//  * Main dialog for adjusting reservation stay (extension or reduction)
//  * ✅ Supports both pre-check-in and checked-in reservations
//  * ✅ FIXED: Proper alert notifications
//  */
// function show_adjust_stay_dialog(frm) {
//     // First, fetch current reservation status
//     frappe.call({
...
//                     // Build info message based on check-in status
//                     let info_message = `<div class="alert alert-info">
//                         <strong>Adjust Stay Duration</strong><br/>`;
                    
//                     if (is_checked_in) {
//                         info_message += `✓ <strong>${checked_in_count} room(s) currently checked in</strong><br/>
//                         You can extend or reduce the checkout date. All checked-in rooms will be affected.`;
//                     } else {
//                         info_message += `
//                         <strong>Pre-Check-In Reservation</strong><br/>
...
//                             // ═══════════════════════════════════════════════
//                             // CURRENT STAY INFORMATION (READ-ONLY)
//                             // ═══════════════════════════════════════════════
//                             {
//                                 fieldtype: 'Section Break',
//                                 label: __('Current Stay Information')
//                             },
//                             {
...
//                             },
//                             {
//                                 label: __('Current Check-Out Date'),
...
//                             {
//                                 fieldtype: 'Section Break',
//                                 label: __('Rooms & Pricing')
//                             },
//                             {
//                                 label: __('Total Rooms'),
//                                 fieldname: 'total_rooms',
//                                 fieldtype: 'Int',
//                                 default: frm.doc.total_rooms || 0,
//                                 read_only: 1
//                             },
//                             {
//                                 fieldtype: 'Column Break'
//                             },
//                             {
//                                 label: __('Rooms Checked In'),
//                                 fieldname: 'checked_in_rooms',
//                                 fieldtype: 'Int',
//                                 default: checked_in_count || 0,
//                                 read_only: 1
//                             },
//                             {
//                                 fieldtype: 'Column Break'
//                             },
//                             {
//                                 label: __('Current Subtotal'),
//                                 fieldname: 'current_subtotal',
//                                 fieldtype: 'Currency',
//                                 default: frm.doc.subtotal || 0,
//                                 read_only: 1
//                             },
...
//                                 onchange: function () {
//                                     update_adjustment_calculations(d, frm, default_checkout_time, is_checked_in);
//                                 }
//                             },
//                             {
//                                 fieldtype: 'Column Break'
...
//                                 description: __("Time for new check-out")
//                             },
...
//                                 fieldtype: 'Data',
//                                 read_only: 1,
//                                 description: __('Extension (more nights), Reduction (fewer nights), or No Change')
//                             },
//                             {
//                                 fieldtype: 'Column Break'
//                             },
...
//                             },
//                             {
//                                 label: __('Night Difference'),
//                                 fieldname: 'nights_difference',
//                                 fieldtype: 'Int',
//                                 read_only: 1,
//                                 description: __('Positive = Extension, Negative = Reduction')
//                             },
...
//                                 fieldtype: 'Currency',
//                                 read_only: 1,
//                                 description: __('Positive = Additional charge, Negative = Refund due')
//                             },
//                             {
//                                 fieldtype: 'Column Break'
//                             },
//                             {
//                                 label: __('New Subtotal'),
//                                 fieldname: 'new_subtotal',
//                                 fieldtype: 'Currency',
//                                 read_only: 1
//                             },
//                             {
...
//                             {
//                                 fieldtype: 'Section Break',
//                                 label: __('Discount Adjustment (Optional)')
//                             },
//                             {
...
//                                 fieldtype: 'Currency',
//                                 default: frm.doc.discount_amount || 0,
//                                 description: __('Adjust discount for new stay duration (optional)'),
//                                 onchange: function () {
//                                     update_adjustment_calculations(d, frm, default_checkout_time, is_checked_in);
//                                 }
//                             }
...
//                                 frappe.msgprint({
//                                     title: __("Invalid"),
//                                     message: __("Please select new checkout date."),
//                                     indicator: "orange"
//                                 });
//                                 return;
//                             }
...
//                             const new_dt = frappe.datetime.str_to_obj(new_checkout + " " + new_checkout_time);
//                             const current_dt = frappe.datetime.str_to_obj(frm.doc.to_date + " 12:00:00");
...
//                                     indicator: "red",
//                                     message: __("New checkout must be after check-in date: {0}",
//                                         [frappe.datetime.global_date_format(frm.doc.from_date)])
//                                 });
//                                 return;
//                             }

//                             // For checked-in: cannot be in past
//                             if (is_checked_in && new_dt < now_dt) {
//                                 frappe.msgprint({
//                                     title: __("Invalid Date"),
//                                     indicator: "red",
//                                     message: __("New checkout cannot be in the past.")
//                                 });
//                                 return;
//                             }

//                             // Must be different from current
//                             if (new_checkout === frm.doc.to_date) {
//                                 frappe.msgprint({
//                                     title: __("No Change"),
//                                     message: __("New checkout date is the same as current. No adjustment needed."),
//                                     indicator: "blue"
//                                 });
//                                 return;
//                             }
...
// /**
//  * Update calculations in real-time as user changes dates
//  */
...
//     const new_checkout_time = d.get_value('new_checkout_time') || default_checkout_time;
//     const new_datetime = new_checkout_str + " " + new_checkout_time;

//     // Parse dates
//     let current_checkout = frm.doc.to_date + " 12:00:00";
//     let current_dt = frappe.datetime.str_to_obj(current_checkout);
//     let selected_dt = frappe.datetime.str_to_obj(new_datetime);
//     let checkin_dt = frappe.datetime.str_to_obj(frm.doc.from_date + " 00:00:00");
...
//         type = 'Extension';
//         type_color = '#0056b3';  // Blue
//     } else if (selected_dt < current_dt) {
//         type = 'Reduction';
//         type_color = '#d9534f';  // Orange/Red
//     } else {
//         type = 'No Change';
//         type_color = '#6c757d';  // Grey
//     }
...
//     // Calculate nights
//     let diff_days = frappe.datetime.get_day_diff(new_checkout_str, frm.doc.from_date);
...
//     let current_subtotal = frm.doc.subtotal || 0;
//     let new_subtotal = 0;
    
//     frm.doc.rooms.forEach(room => {
//         let rate = room.rate_per_night || 0;
//         let new_room_total = rate * diff_days;
...
//     // Amount change
//     let amount_change = new_subtotal - current_subtotal;
...
//     // Final total
//     let final_amount = new_subtotal - new_discount;
...
//  * ✅ FIXED: Proper alert and message notifications
//  */
// function proceed_with_adjustment(frm, values, d, is_checked_in) {
//     const new_checkout = values.new_checkout;
//     const new_checkout_time = values.new_checkout_time || "12:00:00";
//     const new_discount = values.new_discount || 0;
...
//         callback: (r) => {
//             console.log("Adjustment Response:", r);  // Debug log
...
//                 frappe.msgprint({
//                     title: __("Error"),
//                     message: __("An error occurred while adjusting the stay. Please check the error log."),
//                     indicator: "red"
//                 });
//                 return;
//             }
...
//                 // ✅ FIXED: Show toast alert first
//                 frappe.show_alert({
//                     message: __("Stay {0} completed successfully!", [result.adjustment_type]),
//                     indicator: 'green'
//                 }, 7);

//                 // Build detailed success message
//                 let status_label = is_checked_in ? "Checked In" : "Pre-Check-In";
//                 let message = `<div style="padding: 10px;">
//                     <strong style="font-size: 1.1em; color: #28a745;">✓ Stay ${result.adjustment_type} Completed (${status_label})</strong>
//                     <table style="margin-top: 15px; width: 100%; border-collapse: collapse;">
//                         <tr style="border-bottom: 1px solid #ddd;">
//                             <td style="padding: 8px; font-weight: bold;">Old Checkout:</td>
//                             <td style="padding: 8px; text-align: right;">${result.old_checkout}</td>
//                         </tr>
//                         <tr style="border-bottom: 1px solid #ddd;">
//                             <td style="padding: 8px; font-weight: bold;">New Checkout:</td>
//                             <td style="padding: 8px; text-align: right;">${result.new_checkout}</td>
//                         </tr>
//                         <tr style="border-bottom: 1px solid #ddd;">
//                             <td style="padding: 8px; font-weight: bold;">Old Nights:</td>
//                             <td style="padding: 8px; text-align: right;">${result.old_nights}</td>
//                         </tr>
//                         <tr style="border-bottom: 1px solid #ddd;">
//                             <td style="padding: 8px; font-weight: bold;">New Nights:</td>
//                             <td style="padding: 8px; text-align: right;">${result.new_nights}</td>
//                         </tr>
//                         <tr style="background-color: #f0f0f0; border-bottom: 1px solid #ddd;">
//                             <td style="padding: 8px; font-weight: bold;">Night Change:</td>
//                             <td style="padding: 8px; text-align: right; color: ${result.night_difference > 0 ? '#0056b3' : '#d9534f'}; font-weight: bold;">
//                                 ${result.night_difference > 0 ? '+' : ''}${result.night_difference}
//                             </td>
//                         </tr>
//                         <tr style="border-bottom: 1px solid #ddd;">
//                             <td style="padding: 8px; font-weight: bold;">Old Total:</td>
//                             <td style="padding: 8px; text-align: right;">${format_currency(result.old_total)}</td>
//                         </tr>
//                         <tr style="border-bottom: 1px solid #ddd;">
//                             <td style="padding: 8px; font-weight: bold;">New Total:</td>
//                             <td style="padding: 8px; text-align: right;">${format_currency(result.new_total)}</td>
//                         </tr>
//                         <tr style="background-color: ${result.amount_change > 0 ? '#e3f2fd' : '#fce4ec'};">
//                             <td style="padding: 10px; font-weight: bold; font-size: 1.05em;">Amount ${result.is_extension ? 'Charge' : 'Refund'}:</td>
//                             <td style="padding: 10px; text-align: right; font-weight: bold; font-size: 1.05em; color: ${result.amount_change > 0 ? '#0056b3' : '#d9534f'};">
//                                 ${result.amount_change > 0 ? '+' : ''}${format_currency(Math.abs(result.amount_change))}
//                             </td>
//                         </tr>
//                     </table>`;

//                 if (result.additional_invoice) {
//                     message += `<div style="margin-top: 15px; padding: 10px; background-color: #fff3cd; border-radius: 4px;">
//                         <strong>✓ Additional Invoice Created:</strong> <a href="/app/sales-invoice/${result.additional_invoice}">${result.additional_invoice}</a>
//                     </div>`;
//                 }

//                 if (result.credit_note) {
//                     message += `<div style="margin-top: 15px; padding: 10px; background-color: #d4edda; border-radius: 4px; color: #155724;">
//                         <strong>✓ Credit Note Created:</strong> <a href="/app/sales-invoice/${result.credit_note}">${result.credit_note}</a>
//                     </div>`;
//                 }

//                 if (result.is_reduction && result.amount_change < 0 && !result.credit_note) {
//                     message += `<div style="margin-top: 15px; padding: 10px; background-color: #f8d7da; border-radius: 4px; color: #721c24;">
//                         <strong>⚠️ Refund Required:</strong> A refund of ${format_currency(Math.abs(result.amount_change))} is due to the guest. Process this via Payment Entry.
//                     </div>`;
//                 }

//                 message += `<div style="margin-top: 15px; color: #666; font-size: 0.9em;">
//                     <strong>Updated Documents:</strong><br/>
//                     • Hotel Room Reservations: ${result.updated_hrrs}<br/>
//                     • Hotel Room Check-Ins: ${result.updated_checkins}
//                 </div>
//             </div>`;

//                 // ✅ FIXED: Show detailed dialog
//                 frappe.msgprint({
//                     title: __("Adjustment Successful"),
//                     message: message,
//                     indicator: 'green',
//                     wide: true
//                 });
...
//                 }, 500);
//             } else {
//                 // API returned but success is false or missing
//                 let error_message = (r.message && r.message.message) 
//                     ? r.message.message 
//                     : __("Failed to adjust stay. Please try again or contact support.");
...
//             let error_message = __("Failed to adjust stay. Please try again or contact support.");
...
//                                             ? `<span class="badge badge-info"><i class="fa fa-check"></i> ${room.checkin_status}</span>`
...
// Hotel Front Desk Reservation - Client Script v7 (BULK INVOICE CHECK-IN)
// ═══════════════════════════════════════════════════════════════════════════
// ✅ SUPPORTS: Pre-check-in AND checked-in reservations
// ✅ FEATURES: Extension AND reduction with smart validation
// ✅ NEW: Check In Rooms (Bulk Invoice) - check in without creating invoices
// ✅ NEW: Adjustment Invoices tracking display
...
function show_single_guest_edit_dialog(frm, room_idx, parent_dialog) {
    ...
    let d = new frappe.ui.Dialog({
        ...
        fields: [
            ...
            { fieldtype: 'Column Break' },
            { fieldname: 'guest_phone', fieldtype: 'Data', label: 'Phone (Optional)', default: room.guest_phone },
            { fieldtype: 'Section Break', label: 'Info' },
            { fieldtype: 'HTML', fieldname: 'info_html', options: `<div class="alert alert-secondary"><small><strong>Note:</strong> If Email or Phone are left empty, the primary guest email/phone will be used.</small></div>` }
        ],
        size: 'large',
        primary_action_label: __('Save Changes'),
        primary_action: function(values) {
...
function fetch_room_rate(frm, row) {
    frappe.call({
        method: 'rhohotel.api.get_room_rate',
        args: { room_type: row.room_type, check_in_date: frm.doc.from_date },
        callback: function(r) {
            if (r.message) {
                frappe.model.set_value(row.doctype, row.name, 'rate_per_night', r.message);
                if (frm.doc.number_of_nights) frappe.model.set_value(row.doctype, row.name, 'room_total', r.message * frm.doc.number_of_nights);
                calculate_total(frm);
            }
        }
    });
...
function refresh_available_rooms(frm) {
    if (frm.doc.from_date && frm.doc.to_date) {
        frappe.show_alert({ message: __('Dates updated. Click "Add Available Rooms" to see availability'), indicator: 'blue' }, 3);
    }
}
