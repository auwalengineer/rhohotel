...
# 		frappe.throw(
# 			_("Cannot adjust reservation without a sales invoice. Please create an invoice first."),
# 			title=_("No Sales Invoice"),
# 		)
...
# 	# Base amount for the difference in nights (without discount)
# 	base_amount = flt(doc.rate) * diff_nights

# 	# VALIDATION 5
# 	if new_check_in == checkin_dt and new_dt == current_dt:
# 		frappe.throw("No adjustment needed.")

# 	# Use new_discount if provided, otherwise use existing discount
# 	discount_to_use = flt(new_discount) if new_discount is not None else flt(doc.discount)
...
# 							"item_code": doc.room_number,
# 							"qty": diff_nights,
# 							"rate": doc.rate,  # 🔧 Keep original rate
...
# 			if discount_to_use and flt(discount_to_use) > 0:
# 				if discount_type == "Percentage":
# 					invoice.additional_discount_percentage = flt(discount_to_use)
# 				else:  # Amount - calculate proportional discount for extension
# 					# Proportional discount for the additional nights
# 					if doc.number_of_nights > 0:
# 						discount_per_night = flt(doc.discount) / flt(doc.number_of_nights)
# 						extension_discount = discount_per_night * diff_nights
# 						invoice.discount_amount = flt(extension_discount)
...
# 			# 🔧 Apply proportional discount to credit note
# 			if discount_to_use and flt(discount_to_use) > 0:
# 				if discount_type == "Percentage":
# 					credit_note.additional_discount_percentage = flt(discount_to_use)
# 				else:  # Amount
# 					if doc.number_of_nights > 0:
# 						discount_per_night = flt(doc.discount) / flt(doc.number_of_nights)
# 						reduction_discount = discount_per_night * diff_nights
# 						credit_note.discount_amount = flt(reduction_discount)
...
# 		# Update discount if new_discount was provided
...
# 		# 🔧 FIX: Update parent doc using db_set for submitted documents
# 		# Use flags to allow update after submit for date fields
...
# 		# 🔧 FIX: Update discount using db_set if new_discount was provided
# 		if new_discount is not None:
# 			frappe.db.set_value(
