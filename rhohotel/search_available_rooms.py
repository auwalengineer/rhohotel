...
# Handles room availability search and pricing queries
# IMPROVED: Now validates guest capacity across multiple rooms collectively
# """
...
#     Shows ALL available rooms with their details, pricing, and availability.
    
#     IMPROVED: Now considers total capacity of multiple rooms to accommodate guests.
#     For example: 2 adults + 2 rooms of capacity 1 each = OK (total capacity 2)
#     """
...
#                 "message": "No room types are currently available. Please contact the hotel."
#             }
...
#             # === CHECK GUEST CAPACITY (ACROSS ALL REQUESTED ROOMS) ===
...
#             available_count = total_vacant - booked_count
...
#             # === CALCULATE PRICING ===
#             # Calculate price for a single room, then multiply by number of rooms
...
#             # === GET ROOM DETAILS ===
#             room_list = get_available_room_list(room_type_name, check_in_str, check_out_str)
...
#                 "available_rooms": room_list[:num_rooms],  # Show only the number of rooms requested
...
#                 f"The following room types have no pricing configured and were excluded: "
#                 f"{', '.join(rooms_without_tariff)}"
#             )
        
#         if rooms_not_matching_guests:
#             response["warning_guest_mismatch"] = (
#                 f"The following room types cannot accommodate {num_rooms} room(s) for your guest count:\n"
#                 f"{'; '.join(rooms_not_matching_guests)}"
#             )
        
#         # Set status
#         if not available_rooms:
#             response["status"] = "no_availability"
#             response["message"] = "No rooms available for the selected dates and guest count"
#         else:
...
#         frappe.throw(f"Error searching available rooms: {str(e)}")
...
#         total_price = single_room_total * num_rooms  # Multiply by number of rooms
...
# @frappe.whitelist(allow_guest=True)
# def get_occupancy_status(check_in_date=None, check_out_date=None):
#     """Get current/future occupancy status."""
#     try:
#         from frappe.utils import getdate
...
Handles room availability search and pricing queries
IMPROVED: Now validates guest capacity across multiple rooms collectively
FURTHER IMPROVED: Excludes rooms with active check-ins during requested dates
"""
...
def search_available_rooms(check_in_date, check_out_date, num_rooms=1, adults=1, children=0):
    ...
    FURTHER IMPROVED: Excludes rooms with active check-ins during the requested dates.
    A room is considered unavailable if:
    - It has a Hotel Room Reservation that overlaps the requested dates
    - It has an active Hotel Room Check In during the requested dates
    """
    ...
    try:
        ...
        for room_type in room_types:
            ...
            # This excludes rooms that have been checked in but not yet checked out
            ...
            # Calculate available rooms: total vacant minus booked minus checked-in
            # Use set logic to avoid double-counting if a room appears in both tables
            unavailable_count = booked_count + checked_in_count
